...
/**
 * Cron command to run PII data retention cleanup.
 *
...
class PiiCleanupCommand extends HyperfCommand
{
    ...
    {
        parent::configure();
        $this->setDescription('Run PII data retention cleanup (automated IP deletion)');
    ...
    public function handle(): void
    {
        ...
        if ($dryRun) {
            ...
            // In dry run mode, show what would be affected
            $this->info('Retention policy:');
            $this->info('  - Post IPs: 30 days');
            $this->info('  - Post emails: 30 days');
            $this->info('  - Flood log: 24 hours');
            return;
        }

        $this->info('Starting PII retention cleanup...');
        ...
        if (is_string($table) && $table !== '') {
            ...
            $method = match ($table) {
                'posts_ip' => 'purgePostIps',
                'posts_email' => 'purgePostEmails',
                'flood_log' => 'purgeFloodLog',
                default => null,
            };

            if ($method === null) {
                $this->error("Unknown table: {$table}. Valid: posts_ip, posts_email, flood_log");
                return;
            }
